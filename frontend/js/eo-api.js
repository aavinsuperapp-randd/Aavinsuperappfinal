...
// ── Sidebar Toggle (shared across all EO portal pages) ──────────────────
function handleEoSidebarToggle(e) {
  if (e) {
    e.preventDefault();
    e.stopPropagation();
  }
  const sidebar = document.querySelector('.admin-sidebar, .qc-sidebar, #eo-sidebar, #gm-sidebar, #worker-sidebar, #transport-sidebar');
  const main = document.querySelector('.admin-main, .qc-main, .worker-main, .transport-main');
  let overlay = document.querySelector('#sidebar-overlay, #qc-aeo-sidebar-overlay, .sidebar-overlay, .qc-sidebar-overlay');
...
function initEoSidebarToggle() {
  ...
  if (overlay && !overlay.dataset.bound) {
    ...
    overlay.addEventListener('click', closeEoSidebar);
  }

  toggleBtns.forEach(btn => {
    if (!btn.dataset.bound) {
      btn.dataset.bound = 'true';
      btn.addEventListener('click', handleEoSidebarToggle);
    }
  });
  ...
  if (!window.eoResizeBound) {
    window.eoResizeBound = true;
...
window.initGmSidebarToggle = initEoSidebarToggle;
window.initEoSidebarToggle = initEoSidebarToggle;
window.closeEoSidebar = closeEoSidebar;

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initEoSidebarToggle);
} else {
  initEoSidebarToggle();
}
