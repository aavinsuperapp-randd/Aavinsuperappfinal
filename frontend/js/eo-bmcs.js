...
async function loadRoutes() {
  try {
    let routes = [];
    try {
      const fetchFunc = typeof eoFetch === 'function' ? eoFetch : fetch;
...
window.deleteBmc = async function(id) {
  ...
  try {
    ...
    try {
      if (typeof eoFetch === 'function') {
        await eoFetch(`/api/gm/bmcs/${encodeURIComponent(id)}`, { method: 'DELETE' });
        deletedSuccess = true;
      } else {
        const baseUrl = typeof API_BASE_URL !== 'undefined' ? API_BASE_URL : '';
        const token = window.localStorage.getItem('sb-access-token') || '';
...
window.toggleBmcStatus = async function(id, currentlyActive) {
  ...
  try {
    ...
    try {
      if (typeof eoFetch === 'function') {
        await eoFetch(`/api/gm/bmcs/${encodeURIComponent(id)}/toggle`, {
          method: 'PUT',
