// gm-qc-overview.js — GM Portal MACS Readings & QC Overview Logic
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (document.getElementById('header-eo-name')) {
    document.getElementById('header-eo-name').textContent = profile.name || 'General Manager';
  }

  const logoutBtn = document.getElementById('logout-btn');
  if (logoutBtn) logoutBtn.addEventListener('click', handleLogout);

  if (typeof initGmSidebarToggle === 'function') {
    initGmSidebarToggle();
  }
...
async function loadReadingsForDate(dateStr) {
  ...
  try {
    const res = await apiQcAgmGetMacsReadings(dateStr, period);
    ...
    Object.keys(groups1).forEach(rName => {
      ...
      html1 += `
        <tr class="route-header-row" style="background: #1E293B !important; color: #ffffff !important; font-weight: 700;">
          <td colspan="6" style="padding: 10px 16px; background: #1E293B !important; color: #ffffff !important; border-radius: 4px;">
...
window.handleDeleteMacsData = async function() {
  const dateVal = document.getElementById('macs-date-select')?.value;
  
  let confirmMsg = dateVal 
    ? `Are you sure you want to delete imported MACS software data for date: ${dateVal}?`
  ...
  if (dateVal) {
    deleteUrl = `${baseUrl}/api/qc-agm/macs/delete-date?date=${encodeURIComponent(dateVal)}`;
  }
...
window.handleManualSync = async function() {
  ...
  try {
    const result = await eoFetch('/api/admin/macs-api/sync', { method: 'POST' });
