      ...
      <div class="bmc-card ${bmc.is_active === false ? 'inactive-card' : ''}">
        ...
        <div class="bmc-card-body">
          <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px; margin-bottom:4px;">
            ...
            <span style="font-size:0.75rem; background:#f1f5f9; color:#475569; padding:2px 6px; border-radius:4px; font-weight:600; white-space:nowrap;">Code: ${escHtml(bmc.bmc_code || '')}</span>
          </div>
        ...
        <div class="bmc-card-actions">
          ...
          <button class="btn btn-danger btn-sm" onclick="deleteBmc('${bmc.id || bmc.bmc_code}')" title="Delete BMC" style="padding: 0 10px;">🗑️</button>
        </div>
      </div>
    `;
    }).join('');
...
async function saveBmc() {
  ...
  const routeEl = document.getElementById('bmc-route');
  const route_id = routeEl ? (routeEl.value || null) : null;

  console.log('[saveBmc] Starting save. Values:', { bmc_code, name, district, contact, capacityStr, location, latStr, lngStr, route_id });
