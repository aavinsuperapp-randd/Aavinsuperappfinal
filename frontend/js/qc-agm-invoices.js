// qc-agm-invoices.js — QC AGM Desk: Invoices Tab (Tanker Milk Despatch Advice)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (document.getElementById('header-agm-name')) {
    document.getElementById('header-agm-name').textContent = profile.name || 'QC AGM';
  }
...
async function loadInvoices() {
  ...
  try {
    ...
    const res = await qcAgmFetch(`/api/gm/invoices${qs}`);
      ...
      if (currentSearchQuery) {
        ...
        tbody.innerHTML = `<tr><td colspan="5" class="py-4 text-center">
          <div class="ui-state-card ui-state-empty" style="border:none;box-shadow:none;margin:0;padding:24px 16px;">
            ...
            <button type="button" class="btn-date-filter" onclick="setFilterDate('all')" style="background:#0D9488;color:#FFF;border-color:#0F766E;">📅 View All Invoices</button>
          </div>
        </td></tr>`;
      } else {
        tbody.innerHTML = `<tr><td colspan="5" class="py-4 text-center">
    ...
    tbody.innerHTML = invoices.map((inv, idx) => {
      ...
      return `
        <tr style="cursor:pointer;" onclick="openInvoicePreview('${inv.visit_id}')">
          ...
          <td><strong style="color:#0F766E;">${esc(inv.bmc_code || '—')}</strong></td>
...
async function openInvoicePreview(visitId) {
  ...
  try {
    const res = await qcAgmFetch(`/api/gm/invoices/${visitId}`);
    ...
    body.innerHTML = `
      ...
      <div class="inv-detail-grid">
        ...
        <div class="inv-detail-item"><div class="lbl">Serial No.</div><div class="val" style="color:#0F766E;font-weight:800;">${esc(v.invoice_serial_no || '—')}</div></div>
