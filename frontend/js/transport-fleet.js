...
async function loadFleetData() {
  try {
    const data = await apiGetTransportFleet();
...
async function loadMileageData() {
  ...
  try {
    const url = `/api/transport/mileage?status_filter=${statusFilter}&from_date=${fromDate}&to_date=${toDate}&driver_id=${driverId}&vehicle_id=${vehicleId}`;
    const data = await transportFetch(url);
